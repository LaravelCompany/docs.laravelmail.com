...
This page covers the Package installation. If you want to install SendPortal as a stand-alone application, then head over to the [Installation guide](/getting-started/installation).
...
```bash
php artisan vendor:publish --provider=LaravelCompany\\Mail\\SendportalBaseServiceProvider
```
...
In order to support multi-tenancy, Laravel Mail requires a "Workspace ID" for the session.
...
### Web Routes (Required)
To access SendPortal's application resources (e.g. Campaigns, Subscribers, Templates, Messages etc) from the browser, you must register Laravel Mail's web routes.
